...
TEMM = function(Xn, u, K, initial=NULL, iter.max=500, stop=1e-3, trueY=NULL){
  ...
  for(iter in 1:iter.max){
    ...
    if(is.null(trueY)){
      cat(paste0("iter",iter),Mu_err,t_iter,"\n",sep=" ")
    } else {
      id_err = cluster_err(K,trueY,id_env)$cluster_err
      cat(paste0("iter",iter),Mu_err,id_err,t_iter,"\n",sep=" ")
    }
  ...
  #id_env = apply(eta.est, 1, which.max)
  return(list(id=id_env, pi=pi.est,
